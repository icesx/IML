...
def LocallyWeightedLinearRegression(x,y,theta,tau,i):
    ...
    w_sqrt = np.sqrt(w)
    theta = np.dot(np.dot(np.linalg.inv(np.dot((w_sqrt * X).transpose(),w_sqrt * X)),(w_sqrt * X).transpose()),
                   w_sqrt * y)
    h = np.dot(X,theta)
    plt.figure(1)
...
ini_theta = np.array(
    [[0.2],[0.2],[0.2],[0.2]])  # the number of element in ini_element stands for the degree of the polynomial
